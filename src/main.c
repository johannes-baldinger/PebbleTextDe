...
static void update_time(struct tm* t) {
  ...
  text_layer_set_text(s_data.label, strcat(s_data.buffer,"m"));
  text_layer_set_text(s_data.labelBold, strcat(s_data.bufferHours,"h"));
}
...
static void do_init(void) {
  ...
  GFont fontBold = fonts_get_system_font(FONT_KEY_GOTHIC_14_BOLD );
  ...
  s_data.labelBold = text_layer_create(GRect(0, 0, frame.size.w, 39));
  text_layer_set_background_color(s_data.labelBold,GColorWhite );
  text_layer_set_text_color(s_data.labelBold,GColorBlack );
